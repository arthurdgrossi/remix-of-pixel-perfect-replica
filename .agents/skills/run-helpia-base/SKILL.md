...
`.Codex/skills/run-helpia-base/smoke.mjs`.
...
2. In another shell, run the smoke driver. It asserts HTTP 200 + that the SSR
   ...
   next to the driver:
   ```powershell
   node .Codex/skills/run-helpia-base/smoke.mjs
   ```
   Expected output:
   ```
   ...
   ```
3. **Open `.Codex/skills/run-helpia-base/screenshot-landing.png` and look at
...
Pass a different URL if the port differs: `node .Codex/skills/run-helpia-base/smoke.mjs http://localhost:3000`.
