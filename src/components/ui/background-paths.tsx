...
/**
 * Animated field of flowing contour lines (SVG paths). Decorative background
 * layer — `pointer-events-none`. Stroke color follows `currentColor`, so tint
 * it with a text-color class via `className` (defaults to slate/white).
 *
 * Motion-aware: on the server, first paint, or when the user prefers reduced
 * motion, it renders the paths in their final static state (no looping) — this
 * also keeps SSR and hydration in agreement. Adapted to this app's animation
 * engine (`motion/react`, not `framer-motion`).
 */
...
/**
 * Full hero showcase from the original component (faithful, adapted to
 * `motion/react` + the project's <Button>). Used by the preview route; the
 * landing page instead embeds <FloatingPaths /> directly into its hero.
 */
